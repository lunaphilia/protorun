...
6.  [式](language-spec/06-expressions.md)
    ...
    *   6.6 効果操作呼び出し式
    *   6.7 メンバーアクセス式
7.  [所有権システム](language-spec/07-ownership.md) **(将来フェーズ)**
     *   7.1 所有権
     *   7.2 借用
     *   7.3 ライフタイム
     *   7.4 リソース管理
...
- 2026-03-20: MVP仕様完成
  - EBNF文法と散文仕様の11箇所の矛盾を解消
  - ループ構文（while/for/loop + break/continue代数的効果）を追加
  - エントリポイント（トップレベル式）を定義
  - 数値型の完全仕様（i8〜i128, u8〜u128, f32/f64）を追加
  - 文字列補間（f"...{expr}..."）を追加
  - イテレータプロトコル（Iterator/Iterable）を追加
  - 最小標準ライブラリ（prelude）を新規作成（第10章）
  - メソッド呼び出しとUFCS規則を追加
  - 所有権システム（第7章）を将来フェーズに延期
  - 全サンプルコードをMVP文法に更新
