...
4.  [宣言](language-spec/04-declarations.md)
    ...
    *   4.3 関数定義 (`fn`)
    *   4.4 型定義 (`type`, `enum`)
     *   4.5 効果インターフェースと効果実装の定義 (`effect`, `impl`)
...
- vX.X（YYYY-MM-DD）: 文法の最適化と一貫性改善
  - ヴァリアント型のセパレータを `,` から `|` に変更
  - `handler` キーワードを廃止し `impl` に統一
  - Map/Set リテラル・内包表記を廃止しコンストラクタ方式に変更
  - `TupleExpr` の EBNF 定義を修正（`TypeList` → `ExpressionList`）
  - `EffectOperationCallExpr` を EBNF から削除
  - `elif` を廃止し `else if` に統一
  - パイプ演算子 (`|>`, `|>*`, `>>>`, `>>>*`) を全削除
