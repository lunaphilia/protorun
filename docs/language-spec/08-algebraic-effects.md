...
// ファイルシステム効果インターフェース定義
let FileSystem = effect {
  let open: (path: String, mode: FileMode) -> Result[FileHandle, IOError]
  let close: (handle: FileHandle) -> Result[Unit, IOError]
  let read: (handle: FileHandle) -> Result[String, IOError]
  let write: (handle: FileHandle, content: String) -> Result[Unit, IOError]
}
...
let FileSystemHandler = handler FileSystem for FileSystemConfig {
  let open = fn (self, path: String, mode: FileMode) -> Result[FileHandle, IOError] = {
...
let processFiles = fn (effect localFs: FileSystem, effect remoteFs: FileSystem) -> Result[Unit, Error] = {
  let handle1 = match localFs.open("local.txt", FileMode.Read) {
    Result.Ok(h) => h,
    Result.Err(e) => return Result.Err(e)
  }
  let content = match localFs.read(handle1) {
    Result.Ok(c) => c,
    Result.Err(e) => return Result.Err(e)
  }
  match localFs.close(handle1) {
    Result.Ok(_) => {},
    Result.Err(e) => return Result.Err(e)
  }

  let handle2 = match remoteFs.open("remote/data.zip", FileMode.Write) {
    Result.Ok(h) => h,
    Result.Err(e) => return Result.Err(e)
  }
  match remoteFs.write(handle2, content) {
    Result.Ok(_) => {},
    Result.Err(e) => return Result.Err(e)
  }
  match remoteFs.close(handle2) {
    Result.Ok(_) => {},
    Result.Err(e) => return Result.Err(e)
  }
...
```protorun
// 型定義 (再掲)
let Counter = type { mut count: Int }
...
トップレベル式（プログラムのエントリポイント）で効果操作を使用する場合、`with` 構文で明示的にハンドラを提供する必要があります。暗黙的なデフォルトハンドラは提供されません。

```protorun
// 正しい: with で明示的にハンドラを提供
with console = ConsoleLogger {} {
  console.println("Hello, World!")
}

// エラー: ハンドラなしで効果操作を呼び出すことはできない
// console.println("Hello")  // コンパイルエラー
```
...
let ManagedResource = effect[R] {
  let acquire: fn() -> Result[R, Error] // リソースを獲得
...
let ResourceManager = type[R] {
  ...
  mut resourceInstance: Option[R] = None // 保持するリソースインスタンス
}
// ハンドラ実装
let ResourceManagerHandler = handler ManagedResource[R] for ResourceManager[R] {
  let acquire = fn (self) -> Result[R, Error] = {
    if self.resourceInstance.isSome() then {
      // 既に獲得済みの場合のエラー処理など
    }
    // self.resourceConfig を使ってリソースを生成
    let newResource = match createResource(self.resourceConfig) {
      Result.Ok(r) => r,
      Result.Err(e) => return Result.Err(e)
    }
    self.resourceInstance = Some(newResource)
    // R を返すか、あるいは内部で保持するだけか？ 要検討
    Result.Ok(newResource) // 仮: R を返す
  }

  let useResource = fn (self, resource: R, /* ... */) -> Result[Unit, Error] = {
    // resource を使った処理
  }

  let release = fn (self, resource: R) -> Result[Unit, Error] = {
    // リソースを解放する処理
    match destroyResource(resource) {
      Result.Ok(_) => {},
      Result.Err(e) => return Result.Err(e)
    }
...
> **ループ制御との関係:** ループ構文（`while`, `for`, `loop`）は `break` と `continue` を代数的効果として実装します。詳細は [5. 文](05-statements.md#54-break-と-continue) を参照してください。
