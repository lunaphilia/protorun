...
> ⚠️ **将来フェーズ**: この章の内容は将来フェーズの設計案であり、**現在のMVP仕様には含まれません**。
> Protorun MVP では GC（ガベージコレクション）ベースのメモリ管理を採用しています。
> 所有権システム（借用、ライフタイム、RAII）は将来のフェーズで導入予定です。
> 現在の実装では、この章の構文（`own`, `&`, `&mut`, ライフタイム注釈）は使用できません。
...
ライフサイクル管理効果の詳細については、[8.4 ライフサイクル管理効果](08-algebraic-effects.md#84-ライフサイクル管理効果)を参照してください。
