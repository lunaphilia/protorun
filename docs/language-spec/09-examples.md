...
以下のサンプルプログラムは、Protorun言語の様々な側面を示すために選ばれています。これらの例は、[10. 文法](10-grammar.md)で定義された構文のみを使用しています。
...
let ConsoleLoggerHandler = handler Console for ConsoleLogger {
  let println = (self, message: String) -> Unit = { /* 組み込み */ }
}
...
4. **関数定義**: `(params) -> ReturnType = body` 形式の関数の定義
...
// 使用例
let main = () -> Unit = {
  let compareInt = (a: Int, b: Int) -> Int = a - b
...
let safeSqrt = (x: Int) -> Result[Int, String] = {
  if x < 0 then {
