...
// Effect パラメータを持つ関数
// 'console' という名前で Console 効果の実装に依存
fn log(message: String)(effect console: Console): Unit = {
  console.log(message) // エイリアスを使って操作を呼び出す
}
...
- **Effect パラメータ**: 関数が必要とする効果実装（ハンドラ）への依存性を `effect alias: EffectType` という形でパラメータリストに明示的に宣言します。これにより、関数のシグネチャレベルで効果に関する依存性が明確になります（詳細は [8. 代数的効果](08-algebraic-effects.md) を参照）。
