...
Protorun MVP では、GC（ガベージコレクション）ベースのメモリ管理を採用しています。関数式（クロージャ）は、定義時のスコープにある変数を**参照によってキャプチャ**します。

- **自動キャプチャ**: 関数式の本体で参照される外部変数は、自動的にキャプチャされます。明示的なキャプチャリストは不要です。
- **GCによる管理**: キャプチャされた変数のライフタイムはGCによって管理されます。所有権の移動や借用チェックは行われません（所有権システムは将来フェーズで導入予定）。
- **可変変数のキャプチャ**: `let mut` で宣言された可変変数もキャプチャできます。クロージャ内での変更は外部スコープに反映されます。

```protorun
let x = 10
let addX = fn(y: Int) -> Int = x + y  // x をキャプチャ
let result = addX(5)  // result = 15

let mut counter = 0
let increment = fn() -> Unit = {
  counter = counter + 1  // 可変変数をキャプチャして変更
}
increment()
increment()
// counter = 2
